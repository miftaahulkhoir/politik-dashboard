...
export default function IssueManagement() {
  const [subIssueData, setSubIssueData] = useState([]);
  ...
  useEffect(() => {
    const filteredSubIssue = subIssue.filter((item) => item.issue_id === selectedIssue.id);
    setSubIssueData(filteredSubIssue);
  }, [selectedIssue]);
      ...
      <div className="flex col w-full h-full border-t border-t-neutral-500">
        <div className="w-1/3 overflow-auto flex flex-col border-r border-r-neutral-500">
          <div className="header flex justify-between items-center bg-new-black py-3 px-8">
            ...
            <Button icon={<TbPlus />} className="btn-primary">
              Tambah Isu
            </Button>
          </div>
          <div className="flex flex-col px-4 py-6 cursor-pointer">
            {data?.map((data) => (
              <div
